...
static void handle_cmd_deviceaddr_get(const serial_packet_t *p_cmd)
{
    ble_gap_addr_t ble_gap_addr;
    serial_evt_cmd_rsp_data_deviceaddr_t rsp;

    advertiser_address_default_get(&ble_gap_addr);
    memcpy(rsp.addr, ble_gap_addr.addr, BLE_GAP_ADDR_LEN);
    serial_cmd_rsp_send(p_cmd->opcode, p_cmd->payload.cmd.token,
...
static const serial_handler_common_opcode_to_fp_map_t m_cmd_handlers[] =
{
    ...
    {SERIAL_OPCODE_CMD_DEVICEADDR_GET,          0,                              0,                              handle_cmd_deviceaddr_get},
